...
export async function loadOHLC(symbol, from, to) {
  ...
  const filtered = rows.filter(r=> r.Date >= from && r.Date <= to).map(r=>({
...
export async function simpleBacktest(symbol, from='2020-01-01', to='2024-12-31'){
  ...
  function sma(arr, n, i){ if(i+1 < n) return null; const s = arr.slice(i+1-n, i+1).reduce((a,b)=>a+b,0); return s/n }
  let position = 0; let entryPrice = 0; let cash = 100000; let shares = 0
  const trades = []
  for(let i=0;i<data.length;i++){
    const s = sma(prices, short, i)
    const l = sma(prices, long, i)
    if(s === null || l === null) continue
    const date = data[i].date; const price = data[i].close
    if(s > l && position === 0){
      ...
      cash += shares * price
      trades.push({ type: 'SELL', date, price, shares, pnl: (price - entryPrice) * shares })
      shares = 0
      position = 0
    }
  }
